...
let initialAppState = {input: "",utilisateur:"",text:"", isLoading: false, results: []};
...
module Api = {
  ...
  let decodeItemList = 
  json => json |> Json.Decode.(list(optional(decodeItem)));

  let getResults = (query,uti) =>
    Js.Promise.(
      Fetch.fetch("http://127.0.0.1:8080/message?cours="++query++"&userId="++uti)
      ...
      |> then_(results =>
           ...
           |> List.map(item =>
                switch (item) {
                | Some(item) => item
                | None => {idmessage: "nada", texte: "", dateenvoi: "",idcours:"",auteur:"",nblikes:"",isliked:""}
                }
              )
           |> resolve
         )
...
let reducer = (action, state) =>
  switch (action) {
  | UpdateInput(newInput,second) => ReasonReact.Update({...state, input: newInput,utilisateur:second})
