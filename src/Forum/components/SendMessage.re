 type ctx = {input: string,utilisateur:string,text:string};
  type answer={
success:string,
  };
  let decodemessage =json=>
  Json.Decode.{
success:json |> field("success",string),
  };
...
let texte=ReactDOMRe.Style.make(~width="100%",~height="85%",~border="none",());
   module Styles = {
     let card =
   ...
   let make = (~ctx: ctx, ~appSend, _children) => {
     ...
     render: _self =>
       <form 
         ...
         />
       <button style=bouton type_="submit"> {ReasonReact.string("ENVOYER")} </button>
